...
1. **Read the brain.** Build a snapshot:
   `sh "${CLAUDE_PLUGIN_ROOT}/skills/meditate/scripts/snapshot.sh" brain/ /tmp/brain-snapshot-ruminate.md`.
...
3. **Extract** them into readable batches:
   ...
   ```bash
   python3 "${CLAUDE_PLUGIN_ROOT}/skills/ruminate/scripts/extract-conversations.py" "$CONV_DIR" "$OUT_DIR" --batches N
   ```
...
- `${CLAUDE_PLUGIN_ROOT}/skills/meditate/scripts/snapshot.sh` — builds the brain
