...
3. **The brain structure** — `.afk/brain/` for learnings and `agent_docs/` for task-specific instructions, so knowledge has a place to live.
...
| 4 | **Dev server / run command** | How does the app start? What URL? Needed by `afk:qa`. |
| 5 | **Git hygiene** | Clean default branch? Can worktrees/branches be created? Does CI run the checks? |
| 6 | **E2E/browser capability** | Playwright/agent-browser available? (Optional, but `afk:qa` needs a running app.) |
...
- **Progressive disclosure for everything task-specific.** Deep instructions go to `agent_docs/<topic>.md`; CLAUDE.md carries a one-line pointer per doc ("Read agent_docs/database_schema.md before touching the schema"). Pointers, not copies.
...
incident (those belong in lint rules or `agent_docs/`).
...
agent_docs/            # progressive-disclosure docs, pointed to from CLAUDE.md
```

The split — and say this to the user:

- **`agent_docs/`** = instructions: how to do task X here (building, testing patterns, schema work, service architecture). Written deliberately, read on demand via CLAUDE.md pointers.
- **`.afk/brain/`** = learnings: gotchas, failed approaches, architectural reasoning that can't be a rule. Written by `afk:reflect`, one topic per file, `[[wikilinked]]`, index injected automatically at session start.

Seed the brain with `index.md` (the hook maintains it) — don't pre-write empty
placeholder notes; the quality bar in `afk:reflect` decides what gets in.
If Part 1 or 2 surfaced a real gotcha (a quirky build step, a flaky test, a
non-obvious boundary), write it as the first brain note now.
...
| "More detail in CLAUDE.md = better onboarded agent" | Non-universal content teaches the model to ignore the whole file. Pointers to agent_docs/, not content. |
...
- **afk:reflect** routes learnings into the structure built here: lint rules → Part 1's config, conventions → CLAUDE.md (sparingly), how-tos → `agent_docs/`, gotchas → `.afk/brain/`.
