...
Do not use this skill to *apply* fixes — that's `afk:simplify` (cleanup) or
`afk:implement` (changes). Review only diagnoses. For evidence-based ship/no-ship
verification of a running flow, use `afk:qa`.
