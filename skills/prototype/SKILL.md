...
Prototype is optional and user-driven. It is not a required `afk:ship` phase,
...
- Normal implementation of an already-decided feature; use `afk:implement`.
- A planning interview where the user needs decisions, trade-offs, contracts, or
  source-of-truth conflicts resolved; use `afk:grill`.
...
7. When the answer is known, delete the throwaway shell or fold the winning idea
   into real code through `afk:implement`.
...
- The user wants the prototype shipped as production code. Route that through a
  normal `afk:implement` task instead.
