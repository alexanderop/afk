...
- Creates (only if missing): `brain/`, `brain/index.md`, `brain/principles.md` + `brain/principles/`, `brain/context.md` (the domain glossary `afk:grill` grows), `brain/codebase.md` + `brain/codebase/` (durable as-is maps `afk:map-codebase` authors), `brain/decisions/` + `brain/decisions/index.md` (ADRs `afk:grill` records), and `brain/plans/` + `brain/plans/index.md`.
