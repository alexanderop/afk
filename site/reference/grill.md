...
**Output artifact:** `brain/plans/<slug>.md` when a brain vault exists, otherwise `docs/plans/<slug>.md` — a plan with decisions, contracts, wave-sequenced tasks, and an optional `## Acceptance` bar for experience-bearing work. This plan is the direct input to `afk:implement` or `afk:batch`.
