...
definitions, and hooks. The test suite is built to protect that product at four
...
- **Agent frontmatter** — the two named agents pin their tier and tool allowlist
  ...
  `sonnet`, with `Bash`/`Edit`/`Write`). Omitting `model` would silently inherit
  the user's default tier; omitting `tools` would silently inherit *all* tools
...
This is the check to run after every edit — it is fast, free, and catches the
overwhelming majority of regressions (broken frontmatter, dead links, a skill
...
`--plugin-dir .` and confirms the plugin actually loads: a `system/init` event is
...
- **Routing cases** (`kind: "routing"`) — code-graded: the output must contain
...
| `AFK_EVAL_SKILL` | — | run only one skill's evals |
| `AFK_EVAL_ID` | — | run only one eval id |
